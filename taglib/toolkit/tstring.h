...
namespace TagLib {
  ...
  private:
     ...
     */
    void copyFromLatin1(const char *s, std::size_t length);

    /*!
     * Converts a \e UTF-8 string into \e UTF-16(without BOM/CPU byte order)
     * and copies it to the internal buffer.
     */
    void copyFromUTF8(const char *s, std::size_t length);
     ...
     */
    void copyFromUTF16(const wchar_t *s, std::size_t length, Type t);

    /*!
     * Converts a \e UTF-16 (with BOM), UTF-16LE or UTF16-BE string into
     * \e UTF-16(without BOM/CPU byte order) and copies it to the internal buffer.
     */
    void copyFromUTF16(const char *s, std::size_t length, Type t);
